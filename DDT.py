...
def outputDifferenceOfPair(pair, SBoxes):
    firOutput = SBoxes[pair[0]]
    secOutput = SBoxes[pair[1]]
    return firOutput ^ secOutput

#计算DDT表
def DDT(SBoxes, SBoxNumber):
    DDT = creEmpty2DList(256)
    differentialUniformity = 0#用来记录本表中最大的值
    for x in range(0,256):
...
if __name__=="__main__":
    priDDT(DDT(Sboxes,0), 0)
    priDDT(DDT(Sboxes1,1), 1)
